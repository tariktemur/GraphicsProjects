...
std::ostream& operator<<(std::ostream& os, const Camera& c) {
    ...
    os << std::fixed << std::setprecision(6) << "Camera " << c.cameraId << " => pos: " << c.pos << " gaze: " << c.gaze << '\n'
        << "\tu: " << c.u << " v: " << c.v << " w: " << c.w << '\n'
        << std::fixed << std::setprecision(3) << "\tleft: " << c.left << " right: " << c.right << " bottom: " << c.bottom << " top: " << c.top << '\n'
